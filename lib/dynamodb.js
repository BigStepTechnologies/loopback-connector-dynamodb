...
function query(modelName, filter, model, queryString, timeStart) {
            ...
            if (key === hashKey || (globalKeys[key] && globalKeys[key].hash === key) ||
                ...
                if (condition && condition.constructor.name === 'Object') {
                    ...
                    if (operator === "BETWEEN") {
                        ...
                        // Balram: Fix IN comparator query
                        var ValueExpressions = [];
                        for (var i=0; i<condition.length; i++) {
                          tableParams.ExpressionAttributeValues[ValueExpression + "_" + i] = condition[i];
                          ValueExpressions.push(ValueExpression + "_" + i);
                        }
                        FilterExpression.push(keyName + " " + operator + " (" + ValueExpressions.join(",") + ")");
                    } else {
                        tableParams.ExpressionAttributeValues[ValueExpression] = condition;
                        FilterExpression.push(keyName + " " + operator + " " + ValueExpression);
                    }
    ...
    // Balram: some condition based fixes
...
function scan(model, filter, queryString, timeStart) {
    ...
    // Initialize query as an empty object
    var query = {};
      // Balram: Worked on using Filter Expression instead of legacy ScanFilter 
      let ExpressionAttributeNames = {};
      let ExpressionAttributeValues = {};
      tableParams.ExpressFiionAttributeNames = {};
      tableParams.ExpressionAttributeValues = {};
      let FilterExpression = [];
    ...
    if (FilterExpression.length) {
        ...
        delete tableParams.ExpressionAttributeValues;
        delete tableParams.ExpressionAttributeNames;
    }
...
DynamoDB.prototype.all = function all(model, filter, callback) {
    ...
    if (hashKeyFound === true) {
        ...
        if ((condition && condition.constructor.name === 'Object') || (condition && condition.constructor.name === "Array")) {
            // Balram: We need to make hashKeyFound false, if condition is object or array
            hashKeyFound = false;
            // insideKey = Object.keys(condition)[0];
            // condition = condition[insideKey];
    ...
    if (hashKeyFound === true) {
        ...
        //Balram: We need to make support for limit
        if (limitObjects) {
            tableParams.Limit = limitObjects;
        }
    ...
    // Balram: Uncommented Scan operation code
    else {
      // Call scan function
      ...
      async.doWhilst( function(queryCallback) {
        ...
        }.bind(this));}, function() { return false; }, function (err){
          if (err) {
            callback(err, null);
          } else { 
            if (offset !== undefined) {
